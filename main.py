...
def train(
    ...
    lr_network,                 # initial learning rate for mlp 
    lr_mask,                 # initial learning rate for mlp  
    ...
    print(f'adding mask : {adding_mask}')    
    ...
    # debug_print(f'training freq domain : {training_freq_domain}')
    ...
    # inverse_method = model_kwargs['inverse_method']
    ...
    if model_name != "MipFreqTensorVM":
        optim = torch.optim.Adam(model.get_params(lr_grid, lr_network), betas=(0.9, 0.99))
    else:
        optim = torch.optim.Adam(model.get_params(lr_grid, lr_network, lr_mask), betas=(0.9, 0.99))
    ...
    for i in pbar:
        ...
        if i in update_alpha_mask_list:
            print(f'### cur_grid_size : {cur_grid_size}')
            # alpha volume size
            if torch.prod(cur_grid_size) < 256**3:
                cur_mask_size = cur_grid_size
            else:
                raise ValueError(f"prod(cur_grid_size) > 256**3 : {cur_grid_size}")
        ...
        if i in upsample_list:
            ...
            optim = torch.optim.Adam(model.get_params(lr_grid*lr_scale, lr_network*lr_scale, lr_mask*lr_scale), betas=(0.9, 0.99))
          

        
        if i == begin_mask:
            debug_print("=====> BEGIN MASK TRAINING ...")
            model.init_masks()
        ...
        if i in adding_mask:
            model.adding_mask()
            lr_mask_adjust = lr_mask_adjust+1

        if i in adjust_mask_lr:
            lr_mask = lr_mask * 0.8
            optim = torch.optim.Adam(model.get_params(lr_grid, lr_network, lr_mask), betas=(0.9, 0.99))
    ...
    if model_name != "MipFreqTensorVM":
        ...
        total_params = sum([p.numel() for params_dict in model.get_params(lr_grid, lr_network, lr_mask) for p in params_dict["params"]])
...
if __name__ == "__main__":
    ...
    assert cfg.model["name"] in ["MipFreqTensorVM"]
